...
from src.inka import image_handler
from src.inka.card import Card


def test_fetch_image_links_if_cards_do_not_have_images():
    card1 = Card(front_md="Some text", back_md="Another text", tags=[], deck_name="deck")
    card2 = Card(front_md="More\n\nText", back_md="Answer text", tags=[], deck_name="deck")

    links = image_handler._fetch_image_links([card1, card2])
...
def test_fetch_image_links_if_cards_have_images():
    ...
    image2 = "![](img2.png)"
    card1 = Card(front_md="Some text", back_md=f"Another text\n\n{image1}", tags=[], deck_name="deck")
    card2 = Card(front_md=f"More\n\n{image2}", back_md="Answer text", tags=[], deck_name="deck")
    expected = {
        image1: [card1],
        image2: [card2]
    }
...
def test_update_image_links_when_no_image_links():
    ...
    image_handler._update_image_links_in_cards({})

    for card in cards:
        assert card.front_md == card.updated_front_md
        assert card.back_md == card.updated_back_md


def test_update_image_links_when_card_has_image_link():
    image = "![](path/to/img1.png)"
    card = Card(front_md=f"Some {image} text", back_md=f"Another text\n\n{image}", tags=[], deck_name="deck")
    image_links = {
        image: [card]
    }
    ...
    image_handler._update_image_links_in_cards(image_links)
...
def test_update_image_links_in_several_cards_with_same_images():
    image1 = "![](path/to/img1.png)"
    image2 = "![](images/image2.png)"
    card1 = Card(front_md=f"{image2}\n\nSome {image1} text",
                 back_md=f"Another text\n\n{image1}\n{image2}",
                 tags=[], deck_name="deck")
    card2 = Card(front_md=f"{image2}Some text {image1}",
...
def test_copy_images_to_with_one_image_link(anki_media, image, path_to_anki_image):
    image_handler._copy_images_to(anki_media, [f"![some image]({image})"])
...
def test_copy_images_to_with_multiple_image_links(anki_media,
    ...
    image_handler._copy_images_to(anki_media, [f"![some image]({image})", f"![some image]({another_image})"])
...
def test_copy_images_to_with_non_existing_image_raises_error(anki_media):
    with pytest.raises(FileNotFoundError):
        image_handler._copy_images_to(anki_media, ['![](/path/to/non-existing.png)'])
...
def test_handle_images_if_no_images_then_fields_remain_same(anki_media):
    card = Card("Some text", "Another text", tags=[], deck_name="deck")

    image_handler.handle_images_in([card], anki_media)

    assert card.updated_front_md == card.front_md
...
def test_handle_images_when_cards_have_images(anki_media, image, another_image):
    ...
    card1 = Card(f"Some text {image_link1}", f"{image_link2} Another text", tags=[], deck_name="deck")
    card2 = Card("Question here", f"And answer here {image_link2}", tags=[], deck_name="deck")
    ...
    image_handler.handle_images_in([card1, card2], anki_media)

    assert expected_front1 == card1.front_md
    assert expected_front2 == card2.front_md
    assert expected_back1 == card1.back_md
    assert expected_back2 == card2.back_md


def test_handle_images_copies_images_to_anki_media(anki_media, image, path_to_anki_image):
    card = Card("Some text", f"Another text ![img]({image})", tags=[], deck_name="deck")
